const DbEditor = {
    ...
    musicRefreshSlots() {
        ...
        let floorCount = 1;
        if (typeof MapEditor !== 'undefined' && MapEditor.mapData && MapEditor.mapData.areas) {
            floorCount = MapEditor.mapData.areas.reduce((sum, a) => sum + a.floors.length, 0);
        }
   ...
   _mTone(ctx, freq, type, duration, isWeird, time) {
    ...
    _mSnare(ctx, duration, isWeird, time, sampleRate) {
        ...
        const gain = ctx.createGain();
        gain.gain.setValueAtTime(isWeird ? 0.15 : 0.08, time);
    ...
    setupControls() {
        ...
        document.getElementById('save-db-btn').addEventListener('click', saveHandler);
        const bottomBtn = document.getElementById('save-db-btn-bottom');
        if (bottomBtn) bottomBtn.addEventListener('click', saveHandler);
    }
};
