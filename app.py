...
def ensure_floor_placements(floor):
    """Ensure all placement keys exist on a floor."""
    p = floor.setdefault("placements", {})
    for key in ["items", "monsters", "chests", "doors", "npcs", "wins", "lore", "traps", "breakwalls"]:
        if key not in p:
            p[key] = {}

def migrate_map_data(data):
    """Convert old flat floor array to areas format if needed."""
    # Already new format
    if isinstance(data, dict) and "areas" in data:
        for area in data["areas"]:
            for floor in area.get("floors", []):
                ensure_floor_placements(floor)
        return data
    
    # Old format: single floor dict
    if isinstance(data, dict) and "grid" in data:
        data = [data]
    
    # Old format: flat array of floors
...
def load_map():
    if os.path.exists(MAP_FILE):
        with open(MAP_FILE, 'r') as f:
            data = json.load(f)
            return jsonify(migrate_map_data(data))
...
def playtest():
    if os.path.exists(MAP_FILE):
        ...
        map_data = migrate_map_data(map_data)
    else:
